...
void leer_config(t_kernelConfig* cfg, t_config* config) {
    ...
    cfg->estimacionInicial = config_get_double_value(config, "ESTIMACION_INICIAL");
...
void loguear_config(t_kernelConfig* cfg, t_log* loggerKernel){
    ...
    log_info(loggerKernel, "ALFA: %lf", cfg->alfa);
    log_info(loggerKernel, "ESTIMACION_INICIAL: %lf", cfg->estimacionInicial);
