    ...
    while (1) {
        ...
        switch (paquete->codigo_operacion)
        {
            case INIT_IO:
                ...
                break;
            case INIT_PROC:
                int tamanio_random = rand() % (300 - 150 + 1) + 150;
                procesar_syscall_init_proc(logger, "pseudocodigo.txt", tamanio_random);
    ...
    if(!list_is_empty(exec)){
        ...
        if(!list_is_empty (cpus_conectadas)){
            ...
            if(cpu != NULL){
                ...
                t_proceso_en_new* pcb_a_block = list_get(exec, 0);
                if(pcb_a_block->pcb->pid == syscallDump->pid){
                    cpu->disponible = true;
                    pcb_a_block = list_remove(exec, 0);
                    int* contadorBlocked = dictionary_get(pcb_a_block->pcb->mestado, "BLOCKED");
                    if (contadorBlocked) (*contadorBlocked)++;

                    t_temporal* tiempoExec = dictionary_get(pcb_a_block->pcb->mtiempo, "EXEC");
                    if (tiempoExec) temporal_stop(tiempoExec);

                    pcb_a_block->estado = BLOCKED;
                    pcb_a_block->pcb->pc = syscallDump->pc;


                    log_info(logger, "PID (%d) pasa de EXEC a BLOCKED", pcb_a_block->pcb->pid);

                    t_temporal* tiempoBlocked = dictionary_get(pcb_a_block->pcb->mtiempo, "BLOCKED");
                    if (tiempoBlocked) temporal_resume(tiempoBlocked);

                    if (strcmp(cfg.algoritmoCortoPlazo, "FIFO") != 0){
                        temporal_stop (pcb_a_block->inicioRafaga);
                        int64_t milisegundos = 	temporal_gettime(pcb_a_block->inicioRafaga);
                        temporal_destroy(pcb_a_block->inicioRafaga);

                        pcb_a_block->estimacionRafaga = cfg.alfa * (double)milisegundos + (1 - cfg.alfa) * pcb_a_block->estimacionRafaga;

                    }

                    list_add(blocked, pcb_a_block);
                    
                    pthread_mutex_lock(&mutex_pid_susBloqueo);
                    pid_susBloqueo = pcb_a_block->pcb->pid;
                    pthread_mutex_unlock(&mutex_pid_susBloqueo);

                    pthread_mutex_lock(&mutex_pid_dump);
                    pid_dump = pcb_a_block->pcb->pid;
                    pthread_mutex_unlock(&mutex_pid_dump);


                    enviar_paquete_dump(socketMemoriaSyscall, pid_dump);
                    ...
                    if (pthread_create(&hilo_monitoreo, NULL, termina_dump, (void*) datos) != 0) {
                        perror("Error al crear hilo del planificador de Mediano Plazo");
                        exit(EXIT_FAILURE);
                    }
                    pthread_detach(hilo_monitoreo);

                    sem_post(&sem_dumpTerminada);
...
void procesar_syscall_exit(t_log* logger, t_package_syscall_exit* syscallExit){
    if(!list_is_empty(exec)){
        ...
        if(!list_is_empty (cpus_conectadas)){
            ...
            if(cpu != NULL){
                t_proceso_en_new* pcb_a_exec = list_get(exec, 0);
                if(pcb_a_exec->pcb->pid == syscallExit->pid){
                    cpu->disponible = true;
                    pcb_a_exec = list_remove(exec, 0);
...
void procesar_syscall_io(t_package_io* dispositivo_io, t_log* logger) {
    ...
    if(!list_is_empty(exec)){
        ...
        if (!io) {
            ...
            for(int estado = 0; estado<list_size(exec); estado++ ){
                if(!list_is_empty(exec)){
                    if(!list_is_empty (cpus_conectadas)){
                        pthread_mutex_lock(&mutex_cpu_conectadas);
                ...
                if(!list_is_empty (cpus_conectadas)){
                    ...
                    if(cpu != NULL){
                        ...
                        if(pcb_a_exec->pcb->pid == dispositivo_io->pid){
                            ...
                            log_info(logger, "PID (%d) - Bloqueado por IO: %s",  pcb_a_exec->pcb->pid, dispositivo_io->nombre);
        ...
        log_error(logger, "No hay procesos en exec");
    }
}
