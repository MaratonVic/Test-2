    ...
    void mostrar_elemento(char* key, void* value) {
        ...
        if (!list_is_empty(modulo_io->instancias)){
            for (int i = 0; i < list_size(modulo_io->instancias); i++) {
                ...
                log_info(logger, "    - Pid: %d", instancia->pid);
            }
...
void* escuchar_IO(void* arg) {
    ...
    while(1) {
        ...
        // Crear hilo para escuchar esta IO
        pthread_t hilo_io;
        ...
        *fd_ptr = fd_cliente;
        ...
        argumentos->fd = fd_ptr;
        argumentos->nombre = nombre_copia;
        argumentos->logger = logger;

        pthread_create(&hilo_io, NULL, (void*) escuchar_io_individual, argumentos);
...
void* escuchar_io_individual(void* arg) {
    t_args_io* args = (t_args_io*) arg;
    int fd = *(args->fd);
    char* nombre = args->nombre;
    t_log* logger = args->logger;
    char buffer[64];
    t_instancia_io* instancia_fd = NULL;
    while (1) {
        int res = recv(fd, buffer, sizeof(buffer), MSG_PEEK);
        if (res == 0) {
            log_error(logger, "como %d", fd);
            log_warning(logger, "IO desconectada: %s", nombre);

            mostrar_diccionario_io(logger);

            pthread_mutex_lock(&mutex_diccionario_io);
            t_io* io_desconectada = dictionary_get(diccionario_io, nombre);
            pthread_mutex_unlock(&mutex_diccionario_io);

            bool io_removida = false;

            if (list_size(io_desconectada->instancias) == 1){
                pthread_mutex_lock(&mutex_diccionario_io);
                io_desconectada = dictionary_remove(diccionario_io, nombre);
                pthread_mutex_unlock(&mutex_diccionario_io);
                io_removida = true;
            }

            log_error(logger,"%d", pid_io);            
            if (io_desconectada != NULL) {

                for (int i = 0; i < list_size(io_desconectada->instancias); i++) {
                    int* encontrado = list_get(io_desconectada->instancias, i);
                    if (*encontrado == fd) {
                        instancia_fd = list_remove(io_desconectada->instancias, i);
                        break;
                    }
                }
                if (!io_desconectada->ocupado && instancia_fd) {
                    if (!list_is_empty(blocked)) {
                        t_proceso_en_new* pcb_a_exec = buscar_proceso_por_pid(blocked, instancia_fd->pid);
                        if (pcb_a_exec != NULL) {
                            list_remove_element(blocked, pcb_a_exec);
                            log_info(logger, "Proceso %d pasa de BLOCKED a EXIT", instancia_fd->pid);
                            imprimir_mestado(pcb_a_exec->pcb, logger);
                            destruir_proceso_en_new(pcb_a_exec);
                    ...
                    if (!list_is_empty(suspBlocked)) {
                        t_proceso_en_new* pcb_a_exec = buscar_proceso_por_pid(suspBlocked,instancia_fd->pid);
                        if (pcb_a_exec != NULL) {
                            list_remove_element(suspBlocked, pcb_a_exec);
                            log_info(logger, "Proceso %d pasa de SUSP_BLOCKED a EXIT", instancia_fd->pid);
                            imprimir_mestado(pcb_a_exec->pcb, logger);
                            destruir_proceso_en_new(pcb_a_exec);
                        }
                    }
                ...
                while (!queue_is_empty(io_desconectada->cola_bloqueados)) {
                    t_dispositivo_io* bloqueado = queue_pop(io_desconectada->cola_bloqueados);

                    if (!list_is_empty(blocked)) {
                        t_proceso_en_new* pcb_a_exec = buscar_proceso_por_pid(blocked, bloqueado->pid);
                        if (pcb_a_exec != NULL) {
                            list_remove_element(blocked, pcb_a_exec);
                            log_info(logger, "Proceso %d pasa de BLOCKED a EXIT", bloqueado->pid);
                            imprimir_mestado(pcb_a_exec->pcb, logger);
                            destruir_proceso_en_new(pcb_a_exec);
                            sem_post(&sem_exit);
                            sem_post(&sem_procesos_en_new);
                    ...
                    if (!list_is_empty(suspBlocked)) {
                        t_proceso_en_new* pcb_a_exec = buscar_proceso_por_pid(suspBlocked, bloqueado->pid);
                        if (pcb_a_exec != NULL) {
                            list_remove_element(suspBlocked, pcb_a_exec);
                            log_info(logger, "Proceso %d pasa de SUSP_BLOCKED a EXIT", bloqueado->pid);
                            imprimir_mestado(pcb_a_exec->pcb, logger);
                            destruir_proceso_en_new(pcb_a_exec);
                        }
                    }

                    free(bloqueado); // liberar la estructura bloqueada
                }

                if (io_removida) {
                    list_destroy(io_desconectada->instancias);
                    queue_destroy(io_desconectada->cola_bloqueados);
                    free(io_desconectada);
                }
            }
            ...
            log_error(logger, "Error al recibir de IO %s", nombre);
            break;
        }
