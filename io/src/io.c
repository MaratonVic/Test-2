...
int main(int argc, char* argv[]) {
    ...
    while (estado) {
        ...
        paquete = crear_paquete_handshake_syscall_cpu_io_prueba("IO", HANDSHAKE, INST_IO, dispositivo_io->timer, dispositivo_io->pid);
        uint32_t bytes = 0;
        void* a_enviar = serializar_paquete_syscall_cpu_io(paquete, &bytes);

        ssize_t enviados = send(socket_kernel, a_enviar, bytes, 0);
