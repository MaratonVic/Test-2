...
char* cpu_id = NULL;
t_mem_config_msg* config_msj;

void* hilo_escucha_kernel_syscall(void* arg);
...
int main(int argc, char* argv[]) {
    ...
    u_int32_t tam_pagina, niveles_tabla, entradas_tabla;

    recibir_protocolo_memoria(conexion_memoria ,&tam_pagina, &niveles_tabla, &entradas_tabla);

    config_msj = malloc(sizeof(t_mem_config_msg));
    if (!config_msj) {
        log_error(logger_cpu, "No se pudo asignar memoria para config_msj.");
        cerrar_todo();
        return EXIT_FAILURE;
    }
    config_msj->tam_pagina=tam_pagina;
    config_msj->entradas_por_tabla=niveles_tabla;
    config_msj->cantidad_niveles=entradas_tabla;
    ...
    t_args_dispatch* args_dispatch_2= malloc(sizeof(t_args_dispatch));
    args_dispatch_2->socket = conexion_kernel_dispatch;
    args_dispatch_2->logger = logger_cpu;

    // pthread_t hilo_interrupt;
    // pthread_create(&hilo_interrupt, NULL, hilo_escucha_interrupt, NULL);
    // pthread_detach(hilo_interrupt);

    // pthread_t hilo_kernel_dispatch_syscall;
    // pthread_create(&hilo_kernel_dispatch_syscall, NULL, hilo_escucha_kernel_syscall, args_dispatch_2);
    // pthread_detach(hilo_kernel_dispatch_syscall);
...
void* hilo_escucha_kernel_cpu(void* arg){
    ...
    while (estado) {
        ...
        pthread_mutex_lock(&mutex_pc_io);
        pc_io = kernelCpu->pc;
        pthread_mutex_unlock(&mutex_pc_io);
...
void* hilo_escucha_interrupt(void* ) {
    while (1) {
        uint8_t codigo;
        if (recv(conexion_kernel_interrupt, &codigo, sizeof(uint8_t), 0) <= 0) {
            log_error(logger_cpu, "[INTERRUPT] Error al recibir interrupción");
            break;
        }

        if (codigo == INTERRUPCION) {
            hay_interrupt = 1;
            log_info(logger_cpu, "[INTERRUPT] Señal de interrupción recibida");
        }
    }
    return NULL;
}

void recibir_protocolo_memoria(int socket, u_int32_t* tam_pagina, u_int32_t* niveles_tabla, u_int32_t* entradas_tabla) {
    t_paquete* paquete = recibir_paquete(socket);
    if (paquete == NULL || paquete->buffer == NULL) {
        log_error(logger_cpu, "[Memoria] Error al recibir el paquete del protocolo de inicio.");
        return;
    }
    ...
    memcpy(entradas_tabla, buffer->stream + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    memcpy(tam_pagina, buffer->stream + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    memcpy(niveles_tabla, buffer->stream + offset, sizeof(uint32_t));
...
void* hilo_escucha_kernel_syscall(void* arg){
    ...
    while (estado) {
        leerConsola();
        enviar_handshake_cpu_io(socket);
        log_info(logger, "[Dispatch] Enviar segundo handshake");
        leerConsola();
        enviar_handshake_cpu_io(socket);
        log_info(logger, "[Dispatch] Handshake enviado (CPU ID: %s)", cpu_id);
        log_info(logger, "[Dispatch] Handshake IO/CPU enviado (CPU ID: %s)", cpu_id);
        log_info(logger, "[Dispatch] Enviar tercer handshake");
        leerConsola();
        enviar_handshake_cpu_io(socket);
        log_info(logger, "[Dispatch] Handshake enviado (CPU ID: %s)", cpu_id);
        log_info(logger, "[Dispatch] Handshake IO/CPU enviado (CPU ID: %s)", cpu_id);
        log_info(logger, "[Dispatch] Enviar cuarto handshake");
        leerConsola();
        enviar_handshake_cpu_io_teclado(socket);
        log_info(logger, "[Dispatch] Handshake enviado (CPU ID: %s)", cpu_id);
        log_info(logger, "[Dispatch] Handshake IO/CPU enviado (CPU ID: %s)", cpu_id);
        leerConsola();
        enviar_handshake_cpu_io_teclado(socket);
        ...
        log_info(logger, "[Dispatch] Handshake IO/CPU enviado (CPU ID: %s)", cpu_id);
        leerConsola();
        enviar_syscall_exit(socket);
        leerConsola();
        enviar_syscall_dump(socket, pid_io, pc_io);
    }
