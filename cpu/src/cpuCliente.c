...
void enviar_handshake_cpu_io(int socket){
    t_paquete* paquete = crear_paquete_handshake_syscall_cpu_io("impresora", INIT_PROC, INST_IO, 5000000, cpu_id);
    uint32_t bytes = 0;
...
void enviar_handshake_cpu_io_teclado(int socket){
    t_paquete* paquete = crear_paquete_handshake_syscall_cpu_io("teclado", INIT_IO, INST_IO, 5000, cpu_id);
    uint32_t bytes = 0;
    void* a_enviar = serializar_paquete_syscall_cpu_io(paquete, &bytes);
    send(socket, a_enviar, bytes, 0);
    destruir_paquete_syscall_cpu_io(paquete);
...
void enviar_syscall_exit(int socket){
    ...
    t_paquete* paquete = crear_paquete_syscall_exit(INIT_EXIT, cpu_id, pid_io, pc_io);
