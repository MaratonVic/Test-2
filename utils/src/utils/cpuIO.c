...
t_paquete* crear_paquete_handshake_syscall_cpu_io(char* nombre_modulo, uint8_t codigo_operacion,  uint8_t syscall, uint32_t timer, char* nombreCpu) {
    t_package_io handshake;
    ...
    handshake.nombreCpu = string_duplicate(nombreCpu);
    handshake.nombreLargoCpu = strlen(nombreCpu) + 1;

    t_buffer* buffer = malloc(sizeof(t_buffer));
    buffer->size = sizeof(uint32_t) + handshake.nombreLargo + sizeof(uint32_t) + sizeof(uint8_t) + sizeof(uint32_t) +  sizeof(uint32_t) + handshake.nombreLargoCpu;
    ...
    memcpy(buffer->stream + buffer->offset, &handshake.nombreLargoCpu, sizeof(uint32_t));
    buffer->offset += sizeof(uint32_t);

    memcpy(buffer->stream + buffer->offset, handshake.nombreCpu, handshake.nombreLargoCpu);
    buffer->offset += handshake.nombreLargoCpu;
