#include "/home/utnso/Desktop/tp-2025-1c-Bombombini-Gusini/utils/include/utilsHandshake.h"
#include <errno.h>
...
void* serializar_paquete(t_paquete* paquete, uint32_t* total_size_out) {
    uint32_t buffer_size = (paquete->buffer != NULL) ? paquete->buffer->size : 0;
    void* buffer_stream = (paquete->buffer != NULL) ? paquete->buffer->stream : NULL;
    ...
    memcpy(a_enviar + offset, &buffer_size, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    if (buffer_size > 0 && buffer_stream != NULL) {
        memcpy(a_enviar + offset, buffer_stream, buffer_size);
    }
