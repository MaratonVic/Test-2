...
volatile sig_atomic_t hay_interrupt = 0;
...
t_paquete* crear_paquete_syscall_exit(uint8_t codigo_operacion, char* nombre, uint32_t pid, uint32_t pc) {
    ...
    t_buffer* buffer = malloc(sizeof(t_buffer));
    buffer->size = sizeof(uint32_t) + exit.nombreLargo + sizeof(uint32_t) + sizeof(uint32_t);
    buffer->offset = 0;
    buffer->stream = malloc(buffer->size);
    ...
    memcpy(buffer->stream + buffer->offset, &exit.pc, sizeof(uint32_t));
    buffer->offset += sizeof(uint32_t);
...
t_package_syscall_exit* deserializar_sycall_exit(t_buffer* buffer) {
    if (!buffer || !buffer->stream) return NULL;

    void* stream = buffer->stream;
    t_package_syscall_exit* exit = malloc(sizeof(t_package_syscall_exit));
    if (!exit) return NULL;

    memset(exit, 0, sizeof(t_package_syscall_exit));

    if (buffer->size < sizeof(uint32_t) * 3 + 1) {
        free(exit);
        return NULL;
    }
    ...
    if (exit->nombreLargo == 0 || exit->nombreLargo > buffer->size) {
        free(exit);
        return NULL;
    }
    ...
    memcpy(&(exit->pc), stream, sizeof(uint32_t));
...
void* serializar_paquete_syscall_exit(t_paquete* paquete, uint32_t* bytes_out) {
    uint32_t total_size = sizeof(uint8_t) + sizeof(uint32_t) + sizeof(uint32_t) + paquete->buffer->size;
